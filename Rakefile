require "rubygems" require 'rake' require 'yaml' require 'time' SOURCE = "." CONFIG = {
  'version' => "0.3.0",
...
module JB
  class Path
    SOURCE = "."
    Paths = {
      :layouts => "_layouts", themes => "_includes/themes", theme_assets => "assets/themes", theme_packages => "_theme_packages", posts => "_posts"
    }
...
desc "Begin a new post in #{CONFIG['posts']}" task :post do
...
# Usage: rake page name="about.html" You can also specify a sub-directory path. If you don't specify a file extention we create an index.html at the path specified
desc "Create a new page." task :page do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "---"
    post.puts "{% include JB/setup %}"
  end end # task :page desc "Launch preview environment" task :preview do
  system "jekyll serve -w" end # task :preview
# Public: Alias - Maintains backwards compatability for theme switching.
task :switch_theme => "theme:switch" namespace :theme do
  ...
  # Public: Install a theme using the theme packager. Version 0.1.0 simple 1:1 file matching.
  #
  # git - String, Optional path to the git repository of the theme to be installed. name - String, Optional name of the theme you want to install.
  #        Passing name requires that the theme package already exist.
  #
  # Examples
  #
  #   rake theme:install git="https://github.com/jekyllbootstrap/theme-twitter.git" rake theme:install name="cool-theme"
  #
  # Returns Success/failure messages.
  desc "Install theme"
  task :install do
    ...
    # Get relative paths to packaged theme files Exclude directories as they'll be recursively created. Exclude meta-data files.
  ...
  # Public: Package a theme using the theme packager. The theme must be structured using valid JB API. In other words packaging is essentially the reverse of installing.
  #
...
# Internal: Download and process a theme from a git url. Notice we don't know the name of the theme until we look it up in the manifest. So we'll have to change the folder name once we get 
# the name.
#
...
def verify_manifest(theme_path)
  ...
  manifest_file.close
  manifest end def ask(message, valid_options)
  if valid_options
    answer = get_stdin("#{message} #{valid_options.to_s.gsub(/"/, '').gsub(/, /,'/')} ") while !valid_options.include?(answer)
  else
    answer = get_stdin(message)
  end
  answer end def get_stdin(message)
  print message
